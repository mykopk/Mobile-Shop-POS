...
- [ ] **Hardcoded branding** — "Fig Mobile" baked into receipts (`pos/page.tsx:517`, `reservations/page.tsx:354`) instead of company settings; many labels inline instead of `lib/constants.ts`; `"Rs "` literal at `lib/money.ts:4`. → Read company name from settings; move labels to `constants.ts`; move `Rs` into constants.
